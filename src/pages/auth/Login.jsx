...
export default function Login() {
    ...
    async function handleSubmit(e) {
        e.preventDefault();
        if (!formValid) { setError("Corrige los errores del formulario."); return; }
        ...
        try {
            ...
            const target = isPathAllowed(role, from) ? from : (ROLE_HOME[role] || "/admin/tickets/gestionar");
            nav(target, { replace: true });
        } catch (err) {
            const status = err?.status;
            if (status >= 500) setError("Error en el servidor. Intenta más tarde.");
            else if (status === 403 && /inactiv/i.test(err?.data?.detail || err?.detail || "")) setError("Cuenta inactiva");
            else if (status === 401 || status === 400 || status === 404) setError("Credenciales inválidas");
            else setError(err?.data?.detail || err?.detail || err?.message || "Error al iniciar sesión");
        } finally {
            setSubmitting(false);
        }
    }
        ...
        >
            <div className="relative w-full max-w-md">
                <div className="absolute -top-26 left-1/2 -translate-x-1/2 z-10">
                    <img src="/logo_ticket-help.svg" alt="TicketHelp Logo" className="h-46 w-38 object-contain drop-shadow" draggable="false" />
                </div>
                ...
                <div className="bg-white border rounded-2xl shadow-sm px-6 pb-6 pt-14">
                    ...
                    <form className="mt-6 space-y-4" onSubmit={handleSubmit} noValidate>
                        <TextField label="Email" name="email" placeholder="tu@email.com"
                            value={email} onChange={(e) => setEmail(e.target.value)} autoComplete="email" error={emailError || undefined} />
                        <PasswordField name="password" autoComplete="current-password"
                            value={password} onChange={(e) => setPassword(e.target.value)} error={passwordError || undefined} />

                        <div className="flex items-center justify-between text-sm">
                            <label className="flex items-center gap-2">
                                <input type="checkbox" className="h-4 w-4" checked={remember}
                ...
                ${(!formValid || submitting) ? "opacity-60 cursor-not-allowed" : "hover:opacity-90"}`}
